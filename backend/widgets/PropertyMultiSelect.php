...
class PropertyMultiSelect extends Widget
{
    ...
    private function getSelectedItems($list)
    {
        ...
        foreach ($list as $key => $item) {
            if (key_exists($key, $selectedItems)) {
                $result[$key] = $selectedItems[$key];
...
//        }
        ...
        if (self::$count == 1) {
            ...
            $view->registerJs(
                ...
                for(var key in params.selectedItems){
                    \$multiSelect.find('select.list option[value=\"' + key + '\"]').remove();
                    \$multiSelect.find('select.list-hidden option[value=\"' + key + '\"]').eq(0).addClass('hidden');
                }
                \$multiSelect.on('click', 'table a.remove', function(){
                    ...
                    if(params.ajax){
                        ...
                        \$multiSelect.find('.list-hidden option[value=\"' + \$tr.attr('data-id') + '\"]').removeClass('hidden');
                        \$multiSelect.find('select.list').html(\$multiSelect.find('select.list-hidden option:not(.hidden)').clone());
                        \$tr.remove();
                    }
                    return false;
                    ...
                    if(\$this.val() != params.defaultIndex){
                        ...
                        \$clone.find('td label').eq(0).text(\$item.text());
                        \$clone.find('td.value input').attr(\"name\", \"" . $this->name . "[\"+\$this.val()+\"][value]\");
                        \$clone.find('td.in-list input').attr(\"name\", \"" . $this->name . "[\"+\$this.val()+\"][in_list]\");
                        //\$clone.append(\$input);
                        if(params.ajax){
                            var data = params.ajaxData;
                            data.id = \$this.val();
                            ...
                            \$multiSelect.find('.list-hidden option[value=\"' + \$this.val() + '\"]').addClass('hidden');
                            \$item.remove();
                            \$clone.appendTo(\$table);
                        }
                    }
        ...
        echo $this->render('propertyMultiSelect', $params);
    }
}
