...
class AppController extends Controller
{
    ...
    public function actionSetup()
    {
        ...
        if (\Yii::$app->db->createCommand("SHOW TABLES LIKE 'migration'")->execute()){
            \Yii::$app->db->createCommand("DELETE FROM `migration` WHERE `migration`.`version` = 'm999999_999999_load_dump'")->execute();
        }
        \Yii::$app->runAction('dump-migrate/up', ['interactive' => $this->interactive]);

    }

    public function actionSync()
    {
        $path = __DIR__ . '/../../common/migrations/sync/sync.sql';
        \Yii::$app->db_all->createCommand(file_get_contents($path))->execute();
    }
